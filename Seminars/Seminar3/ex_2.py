...
print(f'Количество документов  в первые 6 месяцев: {collection.count_documents(query)}')
query = {'properties.month':{'$gte': '6'}}
print(f'Количество документов  во вторые 6 месяцев: {collection.count_documents(query)}')
...
print(f'Количество документов аварий на дорогах US ROUTE и STATE SECONDARY ROUTE: {collection.count_documents(query)}')

query = {'properties.rdconfigur':{'$all': ['TWO-WAY', 'DIVIDED']}}
print(f'Количество документов аварий во время дождя: {collection.count_documents(query)}')

query = {'properties.rdcondition':{'$ne': 'DRY'}}
print(f'Количество документов аварий не на сухой дороге: {collection.count_documents(query)}')